...
AC_ERROR AcquireImages(acDevice hDevice)
{
	...
	//    eliminating the need for software based packet resend mechanism.  This
	//    next check is to see if TCP streaming is available on the camera, if we
	//    will enable it, oherwise we demonstrate how to enable auto packetsize
	//    negotiation and packet resend for traditional UDP pased GVSP stream.

	acNode hTCPEnableNode = NULL;
	AC_ACCESS_MODE accessModeTCPEnable = 0;

	err = acNodeMapGetNodeAndAccessMode(hNodeMap, "TCPEnable", &hTCPEnableNode, &accessModeTCPEnable);
	if (err != AC_ERR_SUCCESS)
		return err;

	if (accessModeTCPEnable != AC_ACCESS_MODE_NI)
	{
		// The TCPEnable node will tell the camera to use the TCP datastream engine.  When
		//    enabled on the camera Arena will switch to using the TCP datastream
		//    engine. There is no further necessary configuration though to
		//    achieve maximum throughput users may want to set the
		//    "DeviceLinkThroughputReserve" to 0 and also set the stream channel
		//    packet delay "GevSCPD" to 0.
		printf("%sEnable TCP stream\n", TAB1);

		err = acNodeMapSetBooleanValue(hNodeMap, "TCPEnable", true);
		if (err != AC_ERR_SUCCESS)
			return err;
	}
	else
